    ...
    home.activation.mutableFileGeneration = let
      ...
      toCommand = (file:
        let
          ...
          baseName = baseNameOf file.source;
          parentDir = dirOf file.source;
          dirEntries = builtins.readDir parentDir;
          isDirectory = dirEntries.${baseName} == "directory";
          copyCmd = if isDirectory then "cp -r" else "cp";
        in ''
          $VERBOSE_ECHO "${source} -> ${target}"
          $DRY_RUN_CMD ${copyCmd} --remove-destination --no-preserve=mode ${source}/. ${target}/
        '');
